...
function getPromise(number) {
  if (number >= 0) return Promise.resolve('promise that will be fulfilled');
  return Promise.reject(new Error('promise that will be rejected'));
}
...
function getAllResult(promises) {
  ...
  return Promise.allSettled(promises).then((results) => {
    results.forEach((result) => newPromises.push(result.value));
